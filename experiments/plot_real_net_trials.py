...
axes[0].plot(xs, q_rob, label="q")
axes[0].set_ylim(-.001, 0.047)
axes[0].set_title("joint positions")

axes[1].plot(xs, f_rob, label="forces")
axes[1].plot(xs, goal, label="fgoal")
axes[1].set_ylim(-.05, 1.0)
axes[1].set_title("forces")

axes[2].plot(xs, act, label="raw action")
...
fig.suptitle(oname+" | delta position control")
fig.tight_layout()
plt.show()